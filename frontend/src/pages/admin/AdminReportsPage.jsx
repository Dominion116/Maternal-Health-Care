...
export default function AdminReportsPage() {
    ...
    async function load() {
      ...
      try {
        const [a, s, f, m] = await Promise.all([
          ...
          adminService.getSUS({ limit: 200 }),
          adminService.getFeedback({ limit: 200 }),
  ...
  const positiveFeedbackPct = feedback.feedback.length
    ? Math.round((feedback.feedback.filter((f) => f.rating >= 4).length / feedback.feedback.length) * 100)
    : null;
  ...
  const reports = [
    {
      id: "usage",
      title: "Usage Report",
      description: "Totals, daily conversation counts, and top classified intents.",
      icon: BarChart3,
      ...
      onExport: () => downloadCSV(
        ...
        [
          ...
          ...analytics.top_intents.map((i) => [`Intent: ${i.intent}`, i.count]),
        ],
      ),
    },
    {
    ...
    },
    {
      ...
      description: "Intent classifier per-class precision/recall/F1 on the held-out test split.",
      icon: TrendingUp,
      ...
      onExport: () => downloadCSV(
        ...
        (modelMetrics.metrics?.testEvaluation?.perClass || []).map((c) => [c.intent, c.precision, c.recall, c.f1, c.support]),
      ),
    },
    {
      id: "emergency",
      ...
      onExport: () => downloadCSV(
        ...
        analytics.recent_emergencies.map((e) => [e.conversation_id, e.flagged_keyword, e.created_at]),
      ),
    },
  ];

  const badgeVariants = { analytics: "rose", evaluation: "sage", users: "info", ai: "neutral", safety: "error" };
  ...
  if (positiveFeedbackPct != null) {
    findings.push({ text: `${positiveFeedbackPct}% of feedback submissions rated 4 or 5 stars`, positive: positiveFeedbackPct >= 60 });
  }
  findings.push({ text: `${analytics.totals.emergency_messages} emergency-flagged messages detected and handled`, positive: true });
  const worstIntent = (modelMetrics.metrics?.testEvaluation?.perClass || []).slice().sort((a, b) => a.f1 - b.f1)[0];
  if (worstIntent) {
    findings.push({
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Export real study data for the MamaGuide evaluation</p>
      </div>
      ...
      <div className="bg-linear-to-r from-rose-700 to-rose-500 rounded-2xl p-5 text-white">
        ...
        <h2 className="font-display font-bold text-xl mb-4">Study Summary Snapshot</h2>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { label: "Total Users", value: analytics.totals.users },
            { label: "Conversations", value: analytics.totals.conversations.toLocaleString() },
            { label: "Avg SUS Score", value: sus.avg_sus_score ?? "—" },
            { label: "Positive Feedback", value: positiveFeedbackPct != null ? `${positiveFeedbackPct}%` : "—" },
          ].map((item) => (
            <div key={item.label}>
              <p className="font-display font-extrabold text-2xl">{item.value}</p>
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        <div className="space-y-3">
          ...
          {reports.map((report) => (
            <div key={report.id} className="bg-white rounded-2xl border border-gray-200 p-4 flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  ...
                  <Badge variant={badgeVariants[report.badge] || "neutral"} size="sm">{report.badge}</Badge>
                </div>
            ...
            <ul className="space-y-2.5">
              {findings.map((f, i) => (
                <li key={i} className="flex items-start gap-2 text-xs">
                  <span className={`mt-0.5 w-4 h-4 rounded-full flex items-center justify-center shrink-0 ${f.positive ? "bg-green-100 text-green-600" : "bg-amber-100 text-amber-700"}`}>
                    {f.positive ? "✓" : "!"}
                  </span>
              ...
              {findings.length === 0 && (
                <p className="text-xs text-gray-400">Not enough data yet to generate findings.</p>
              )}
            </ul>
          </div>
        </div>
